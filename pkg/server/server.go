...
func setup(ctx context.Context, server *Server) (http.Handler, *schema.Collection, error) {
	...
	handler, err := handler.New(server.RestConfig, sf, server.AuthMiddleware, server.Next, server.Router, server.CssGetter, server.JsGetter, server.ApiUIVersionGetter)
	if err != nil {
		return nil, nil, err
	}
